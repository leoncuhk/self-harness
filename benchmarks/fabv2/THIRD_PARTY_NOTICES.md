# Third-party notices

`data/public.csv` is copied from Vals AI's
[`finance-agent-v2`](https://github.com/vals-ai/finance-agent-v2) repository at
commit `b979786a8f9c49c178a88720ea4bb6fb16cbf818`. The source repository is
licensed under the MIT License, Copyright (c) 2025 Vals AI, Inc.

The generated `questions.json` and `evals/frozen/rubrics.json` preserve public
question and rubric content from that file. They are public development data,
not a hidden test. Answer-aware material and unrelated historical result files
are intentionally not vendored.
