...
Upstream self-harness runs every candidate exactly once per split. On noisy
...
``flaky``
    mixed
``apparatus``
    every repeat failed to measure anything (see :mod:`self_harness.apparatus`)
...
from self_harness.core import CaseOutcome, Experiment, RunLayout, SplitResult, Variant
from self_harness.cost import profile_split
