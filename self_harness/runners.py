...
from self_harness.apparatus import STATUS_APPARATUS, apparatus_kind
...
class PytestRunner:
    ...
    def _build_env(
        ...
        *,
        ...
        env.setdefault("LANGSMITH_TEST_SUITE", f"self-harness-{experiment.name}")
        return env
    ...
    def _base_command(self, experiment: Experiment) -> list[str]:
        ...
        command.extend(["-p", "self_harness_plugin"])
        return command
...
def _same_nodeid(candidate: str, case_id: str) -> bool:
    ...
    parametrized node ids such as ``tests/test_suite.py::test_task[x]`` and
